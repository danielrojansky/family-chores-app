...
// ─── Session helpers ──────────────────────────────────────────────────────────
const saveSession  = (p) => { try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ id: p.id, type: p.type })); } catch {} };
...
// ─── API helper ───────────────────────────────────────────────────────────────
const fetcher = (url) => fetch(url).then((r) => { if (!r.ok) throw new Error('API error'); return r.json(); });
...
export default function App() {
  ...
  const { data: familyConfig, mutate: mutateConfig, isLoading: configLoading } =
    useSWR('/api/data?type=config', fetcher, { refreshInterval: 3000, revalidateOnFocus: true });

  const { data: rawChores, mutate: mutateChores } =
    useSWR('/api/data?type=chores', fetcher, { refreshInterval: 3000, revalidateOnFocus: true });
  ...
  if (selectedProfile && !currentProfile) {
    ...
    return <PinEntry profile={selectedProfile} correctPin={correctPin} onSuccess={() => loginAs(selectedProfile)} onBack={() => setSelectedProfile(null)} />;
  }
